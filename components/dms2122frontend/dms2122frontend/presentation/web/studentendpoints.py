...
class StudentEndpoints():
    ...
    @staticmethod
    def get_answer(auth_service: AuthService) -> Union[Response, Text]:
        """ Handles the GET requests to the student root endpoint.
        ...
        if Role.Student.name not in session['roles']:
            return redirect(url_for('get_home'))
        name = session['user']
        return render_template('student/answer.html', name=name, roles=session['roles'])
    ...
    def get_check(auth_service: AuthService) -> Union[Response, Text]:
        ...
        if Role.Student.name not in session['roles']:
            return redirect(url_for('get_home'))
        name = session['user']
        return render_template('student/check.html', name=name, roles=session['roles'])
